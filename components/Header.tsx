...
const NavItem: React.FC<{
    ...
    return (
        <button
            ...
            className={`relative px-5 py-2.5 text-sm transition-colors duration-200 ease-in-out focus:outline-none rounded-md
                ${isActive ? 'font-bold text-blue-600 dark:text-sky-300' : 'text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-slate-100'}`}
...
const Header: React.FC<HeaderProps> = ({ currentView, setView, scrollProgress }) => {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg z-20 border-b border-gray-200/80 dark:border-slate-700/80">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="hidden md:block">
                        <div ref={navRef} className="relative flex items-center justify-center p-1.5 bg-gray-100 dark:bg-slate-800/50 rounded-xl">
                             <div className="absolute top-1.5 bottom-1.5 bg-white dark:bg-slate-700 rounded-lg shadow-sm transition-all duration-300 ease-in-out" style={magicLineStyle}></div>
                            {navItems.map(item => (
                                <NavItem 
                                    key={item.id}
                                    label={item.label}
             ...
             {/* Scroll Progress Bar */}
            <div className="absolute bottom-[-1px] left-0 h-1 bg-blue-600 dark:bg-sky-500 rounded-r-full transition-all duration-150 ease-linear" style={{ width: `${scrollProgress}%` }} />
        </header>
    );
};
